import { selector, handler } from '../_fns/custom-functions.js';
...
export const searchMovie = (search_movie_url, reloadFn = () => window.location.reload()) => {
  ...
  const movieFinder = async (query) => {
    ...
    const filteredResult = url.filter(movie => movie.title.toLowerCase().include(query.toLowerCase()));

    return new Promise(resolve => {
      debounce(() => resolve(filteredResult), 300)
    });
  }
  ...
  const displayResults = async (event) => {
    ...
    try{
      if(query && query.length !== ''){
        ...
        main.innerHTML = `<p class="info">No movies found!</p>`
      }
    }catch(error){
      main.innerHTML = `<p style="color: hsl(11, 89%, 37%)" class="info">Error fetching movies: ${error.message}</p>`;
    }
  }
